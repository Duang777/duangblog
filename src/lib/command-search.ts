...
function tokenScore(token: string, field: string) {
  ...
  // and `field.includes` stops at the inserted space. Compare against the
  // field without its separators as well, otherwise the label is unreachable
  // by its own name.
  const compact = field.replace(/ /g, "");
  if (compact !== field && compact.includes(token)) return 60;
