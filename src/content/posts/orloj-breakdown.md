...
revisions:
  ...
  - date: 2026-07-27
    note: 按 plain-writing 规则重写正文，代码与图表不动。
---

仓库是 [OrlojHQ/orloj](https://github.com/OrlojHQ/orloj)。README 第一句写 Agents are infrastructure。我按 main 上的代码读完，又对照了 `docs/pages/concepts/architecture.md` 和 `execution-model.md`。下面按三块写。第一块是 Agent 技术本身怎么跑。第二块是后端控制面和 Worker 怎么托管这套运行时。第三块是前端控制台怎么订阅同一份状态。关键路径会配图，并把仓库里的核心片段贴出来。

项目还在活跃开发，1.0 前 schema 会变。下面按仓库现状写。

## 它要解决什么

常见 Agent demo 是一段 prompt 加一个循环。业务里真正烦人的是另一批问题。模型要换提供商，还要做 fallback。工具要鉴权、超时、重试，高风险调用还要人批。多 Agent 交接如果埋在控制流里，事后几乎没法对照。任务所有权不清楚时，进程挂了可能双跑，也可能假装还在跑。

Orloj 把这些提成版本化资源。资源有期望状态，也有 controller、Worker 租约、执行期策略和轨迹。本地可以单进程跑。生产可以换成 Postgres 加多 Worker。资源语义尽量不变，换的是底座。

这套东西偏重。如果你还在验证一句 prompt，读起来会觉得吵。如果你已经撞过交接不可见、工具越权、任务双跑，它的问题意识就对上了。

## 仓库里的进程切分

后端是 Go。模块路径是 `github.com/OrlojHQ/orloj`，`go.mod` 里写的是 Go 1.26.5。入口在 `cmd/`。
...
`orlojworker`（`cmd/orlojworker/main.go`）认领 Task、续租、跑 AgentSystem 图、调模型和工具。消息驱动模式下它会消费 agent inbox，再把 phase、消息、trace 写回 store。它也可以跑 `--single-agent` 的 Job 形态，那是另一条部署路径。

`orlojctl`（`cmd/orlojctl/main.go`）几乎只是壳，真正逻辑在 `cli/`。那里负责 apply、校验、脚手架、密钥、跑任务、审批、日志和指标。
...
`Agent` 资源声明的是角色能力，不是整条业务图。关键字段大致如下。
...
`run` 里最能说明问题的几行如下。原文还包含 contract 和 checkpoint 细节，这里删掉了。当前实现里每步还会先塞一条 user content。模型错误有连续失败熔断。
...
步数、超时、工具候选集、权限、策略都写在声明里，运行时按声明执行。`SetCheckpointing` 支持中断后续跑。工具选择模型在文档里写得很直。`tools[]` 是候选。模型每步选出具体调用。只有被选中且授权的才会执行。未授权会返回 `tool_permission_denied`。

## Agent 技术：多 Agent 图怎么走

单 Agent 循环解决一个角色怎么想和怎么调工具。协作拓扑在 `AgentSystem.spec.graph`。

边有两种写法。一种是旧的 `next` 单边。另一种是现在更推荐的 `edges[]`。条件路由挂在 `condition` 上，对着刚完成 Agent 的 output 求值（`output_contains` / `output_matches` / `output_json_path` 等）。没有条件的边无条件开火。都中不了时走 `default: true`。条件路由要求 message-driven 模式。

Fan-out 打多条下游边。Fan-in 用 `wait_for_all` 或 `quorum`。join 状态在 `Task.status.join_states`。Delegation 是两阶段。先派 `delegates`，收齐后原节点 review，再走正常 `edges`。状态在 `Task.status.delegation_states`。
...
message-driven 模式下，图里从一个 Agent 到下一个 Agent 的交接会落成一条可持久化的消息。`Task.status.messages` 里能看到生命周期，包括 `queued`、`running`、`retrypending`、`waitingapproval`、`succeeded`、`deadletter`。撞上人工审核点时进 `waitingapproval`，直到关联的 `TaskApproval` 有结果。
...
执行模式有两种，资源定义可共用。`sequential` 整图在进程内推。`message-driven` 每个 agent step 进队列。条件路由绑在后者上。文档说两种模式共用同一套图和资源定义，trace、history、output 形态也对齐。本地用 sequential，生产切 message-driven，不必改 YAML。

工具层重点是统一外壳，代码在 `runtime/tool_runtime_*.go`、`tool_runtime_governed.go`、`tool_authorizer.go`。授权结果可以是 allow、deny、approval_required。
...
治理不是独立进程。它是 worker 执行时的 inline 检查。Agent 开跑前先过策略。
...
工具调用前再过 `AgentRole` 和 `ToolPermission`。高风险工具可生成 `ToolApproval`。图上关键节点可走 `TaskApproval`。未授权默认拒绝并进 trace。
...
这套治理有成本。kind 会变多。审批拉长时延。副作用工具多时合理。几乎没副作用时会显得重。

## 后端：API、store、调度怎么托管 Agent 运行时

HTTP 不用 Gin 或 Echo。路由集中在 `api/server.go`，标准库 `http.ServeMux`。资源 CRUD 在 `/v1/...`。并发控制走 `resourceVersion` / `If-Match`。认证模式有 `off` 和 `native`。

存储装配在 `startup/stores.go`。任务认领用 `FOR UPDATE SKIP LOCKED`。这是防多 worker 抢同一行的关键 SQL，在 `store/sql_backend.go`。
...
控制面 event bus 在 `eventbus/`，服务 watch 和调度通知。运行时 agent message bus 在 `runtime/agent_message_bus*.go`，负责图内交接、ack/nack、延迟重试。`docker-compose.yml` 里是 Postgres、NATS `-js`、`orlojd` 和多个 worker。

## 前端：技术栈和打包方式

控制台在 `frontend/`。React 19、TypeScript、Vite 8。状态用 `@tanstack/react-query` 和 `zustand`，图画 `@xyflow/react`，编辑器用 Monaco。样式是手写 CSS。

生产形态用 `go:embed` 打进 `orlojd`。代码如下。
...
实时刷新靠 `EventSource`，代码在 `frontend/src/api/watch.ts`。
...
事件进来后 invalidate React Query。保存 YAML 会跟 `resourceVersion` 较劲。这和后端 `If-Match` 是同一件事的两端。你在 UI 批一个 `ToolApproval`，改的是后端审批资源。worker 侧消息才能离开 `waitingapproval`。

## 周围资源

`ContextAdapter` 在 system 启动前改输入。`TaskSchedule` 和 `TaskWebhook` 管定时和签名事件。`EvalDataset` 和 `EvalRun` 压测 system。`Worker` 声明容量与心跳。`Session` 是并列对话路径。资源面大，但 CLI、REST、UI 和部分 CRD 说同一套对象。

## 可观测性

把模型调用、工具调用、错误、token、延迟、审批、重试收进 Task trace 和 history。再叠 Prometheus、可选 OTel，以及控制台 Trace 和 Timeline。排障入口盯 Task 名更划算。

## 我认可的，和暂时不押的

认可的部分如下。单 Agent 循环把步数、工具候选、权限做成声明。多 Agent 用图边表达协作。`SKIP LOCKED` 租约是数据库语义。治理 fail closed。前端 Query、SSE、embed 跟控制面同进程交付。

暂时不押的部分如下。资源面大。1.0 前 schema 会动。轻量场景可能过重。条件路由绑 message-driven，和本地 sequential 不完全同构。CRD 只覆盖部分 kind。

## 收尾

读 Orloj，我会先盯 `AgentWorker.run` 和 `AgentSystem` 图，再看 claim SQL 与两路总线，最后对照 `watch.ts` 看控制台如何订阅同一份 Task 状态。它想解决的是多智能体离开 demo 之后，声明、调度、边界和痕迹还在。Agent 运行时、控制面、控制台说的是同一套资源。
