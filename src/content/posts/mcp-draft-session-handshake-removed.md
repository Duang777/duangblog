...
<section class="article-embed-note">
  ...
  <div class="article-lanes">
    <span class="article-lane">客户端 Client</span>
    <span class="article-lane">服务端 Server</span>
  </div>
  <ol class="article-timeline">
    <li class="article-event article-event-client">
      <p><code>initialize</code></p>
      <p>带协议版本、客户端能力、clientInfo</p>
    </li>
    <li class="article-event article-event-server">
      <p><code>InitializeResult</code></p>
      <p>回服务端能力、serverInfo</p>
    </li>
    <li class="article-event article-event-client">
      <p><code>notifications/initialized</code></p>
      <p>握手完成，会话正式成立</p>
    </li>
    <li class="article-event article-event-server">
      <p><code>Mcp-Session-Id</code></p>
      <p>后续请求都要带，会话状态绑定在服务端</p>
    </li>
    <li class="article-event article-event-client">
      <p><code>tools/list</code> / <code>tools/call</code></p>
      <p>可按会话身份返回不同工具集</p>
    </li>
    <li class="article-event article-event-server">
      <p>反向请求客户端</p>
      <p><code>roots/list</code> · <code>sampling/createMessage</code> · <code>elicitation/create</code></p>
    </li>
  </ol>
...
<section class="article-embed-note">
  ...
  <div class="article-compare-head">
    <span class="article-chip article-chip-old">旧：会话模型</span>
    <span class="article-chip article-chip-new">新：无状态模型</span>
  </div>
  <dl class="article-compare-list">
    <div>
      <dt>启动方式</dt>
      <dd>必须握手（<code>initialize</code> + <code>notifications/initialized</code>）</dd>
      <dd>无握手，每次请求在 <code>_meta</code> 里自带版本和能力</dd>
    </div>
    <div>
      <dt>状态位置</dt>
      <dd>靠 <code>Mcp-Session-Id</code> 维持，会话状态在服务端</dd>
      <dd>无会话，跨调用状态用显式 handle 参数传递</dd>
    </div>
    <div>
      <dt>交互方向</dt>
      <dd>服务端可反向请求客户端（Roots/Sampling/Elicitation）</dd>
      <dd>改为 MRTR：返回 <code>input_required</code>，客户端补齐后重试</dd>
    </div>
    <div>
      <dt>通知与断流</dt>
      <dd>GET + SSE，可用 <code>Last-Event-ID</code> 续传</dd>
      <dd><code>subscriptions/listen</code>，断流即作废，必须新 ID 重发</dd>
    </div>
    <div>
      <dt>部署特性</dt>
      <dd>需要会话粘滞，滚动重启成本高</dd>
      <dd>天然适合负载均衡、水平扩容、滚动发布</dd>
    </div>
  </dl>
...
随之而来的一个容易被忽略的约束是：「tools/list」、「resources/list」、「prompts/list」这三个列表接口不再随连接变化。过去你可以根据会话的身份给不同客户端露出不同的工具集，现在不行了，同一个 server 对所有调用方给出同一张能力清单。如果你的产品靠「根据登录用户动态生成工具列表」这个能力吃饭，这里得提前想好替代方案，比如按租户拆出不同的端点地址。
...
握手没了，但「我先看看你是谁、支持什么版本」这个需求还在，所以规范新增了「server/discover」这个 RPC，并且写明服务端必须实现。
...
把两类通知分开的好处是：一个无状态的服务，不需要为了发一条进度通知去查「这个客户端的监听流在哪台机器上」。
...
前两条都是无状态化的必然结果：阻塞等待需要服务端持有连接和上下文，轮询则没有这个要求。「tasks/list」被移除同样如此，没有会话就没有「当前连接的任务列表」这个概念。
...
这意味着可靠性的责任从协议转到了实现方身上。如果你的工具带副作用，比如下单、发邮件、写入外部系统，那么幂等设计就不再是可选项。客户端重发时用的是新 ID，服务端无法从协议层得知「这是同一件事的重试」，得靠你自己在参数里带幂等键。

## 九、缓存被写进了规范

这是一组容易被当成「次要改动」跳过去，但对性能影响很大的内容。
...
「ttlMs」是新鲜度提示，单位毫秒，告诉客户端这份结果可以缓存多久，目的是减少无意义的轮询。「cacheScope」取值是 public 或 private，控制共享的中间层能不能缓存这份响应。两者和已有的「listChanged」通知是互补关系，不是替代关系：前者管「多久去看一次」，后者管「变了马上告诉你」。
